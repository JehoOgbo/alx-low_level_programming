 ...
 */
int _strcmp(char *s, int i, int j)
{
	if (s[i] != s[j])
		/* checking case if the converging characters don't match */
		return (0);
	else if (i == j && s[i] == s[j])
		/* checking the middle character in odd-length strings */
		return (1);
...
{
	int length, i, j;
	/* declaring variables to mark the first and last chars in string */

	length = len(s);
	/* calling the len() function to get the value of given string */
	i = 0;
	j = length - 1;
